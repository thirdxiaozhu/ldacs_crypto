...
int main()
{
    ...
    ret = generate_random(sharedinfo_len, sharedinfo); // 用随机数填充
    if (ret != LD_KM_OK)
    {
    ...
    HANDLE handle_rootkey;    // 根密钥句柄
    HANDLE handle_kassgw;     // AS和SGW之间的主密钥
    struct KeyPkg pkg_kassgw; // 用于保存主密钥信息
    uint16_t len_kassgw = 16; // 主密钥长度

    ret = get_rootkey_handle(&handle_rootkey); // 获取根密钥
    if (ret != LD_KM_OK)
    {
    ...
    ret = derive_key(handle_rootkey, MASTER_KEY_AS_SGW, len_kassgw, "SGW", "Berry", sharedinfo, sharedinfo_len, &handle_kassgw, &pkg_kassgw);
    if (ret != LD_KM_OK)
    {
    ...
    ret = generate_random(len_rand_3, rand_3);
    if (ret != LD_KM_OK)
    {
    ...
    ret = derive_key(handle_kassgw, NH_KEY, len_kasgs, "Berry", "GS1", rand_3,
                     len_rand_3, &handle_NH, &pkg_NH); // NH = KDF(KAS-SGW,rand_3)
    if (ret != LD_KM_OK)
    {
    ...
    ret = derive_key(handle_NH, MASTER_KEY_AS_GS, len_kasgs, "Berry", "GS1", xor_result,
                     len_xor_res, &handle_kasgs, &pkg_kasgs); // KAS-GS=KDF(NH,SAC_GS^N3
    if (ret != LD_KM_OK)
    {
    ...
    HANDLE handle_session_key;
    struct KeyPkg pkg_session_key;
    ret = derive_key(handle_kasgs, SESSION_KEY_USR_ENC, len_session_key, "Berry", "GS1",
                     rand_3, len_rand_3, &handle_session_key, &pkg_session_key); // KU-ENC=KDF(KAS-GS,rand3)
     if (ret != LD_KM_OK)
    {
