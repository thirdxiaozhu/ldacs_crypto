...
int main()
{
    ...
    // 更新会话密钥
    uint8_t nonce_len = 16;
    uint8_t nonce[nonce_len];
    generate_random(nonce_len, nonce);
    printf("原会话密钥===================\n");
    print_key_pkg(&sessionkey_pkg); // 原密钥
    struct KeyPkg updated_pkg;
    update_sessionkey(sessionkey_pkg.meta_data, masterkey_handle, nonce, nonce_len, &updated_pkg);
    printf("更新的会话密钥===============\n");
    print_key_pkg(&updated_pkg);
    printf("原会话密钥被撤销==============\n");
    print_key_metadata(sessionkey_pkg.meta_data);
