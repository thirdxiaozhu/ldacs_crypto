...
int main()
{
    ...
    // 查询根密钥id
    QueryResult_for_queryid* qr_mk = query_id(dbname, sgw_tablename, sac_as, sac_sgw, ROOT_KEY, PRE_ACTIVATION);
    if (qr_mk->count != 1)
    {
    ...
    // 查询密钥类型
    log_warn("kid =  %s, kstate %s\n", qr_mk->ids[0], kstate_str(query_state(dbname, sgw_tablename, qr_mk->ids[0])));
}
