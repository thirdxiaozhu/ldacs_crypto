...
int main()
{
    ...
    */
    ...
    // 打印查询结果
    log_warn("Number of subkeys found: %d\n", result.count);
    for (int i = 0; i < result.count; i++) {
        log_warn("Subkey ID %d: %s, Key Type: %s\n", i + 1, result.subkey_ids[i], result.key_types[i]);
    ...
    */
    ...
    void *handle;
        ...
        if (ret != LD_KM_OK)
        {
            log_warn("get_handle_from_db failed: %d\n", ret);
            return 0;
        }
    */
