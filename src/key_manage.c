...
km_hmac(uint8_t *key, uint32_t key_len, uint8_t *data, uint32_t data_len, uint8_t *hmac_value, uint32_t *hmac_len) {
    ...
    do {
        ...
        if (ret != LD_KM_OK) {
            printf("SDF_HashFinal Function failure! %08x \n", ret);
            fail_tag = TRUE;
            break;
...
l_km_err km_rkey_gen_export(const char *as_name, const char *sgw_name, uint32_t key_len, uint32_t validity_period,
    ...
    do {
        ...
        if (key_data == NULL) {
            printf("Failed to create key metadata\n");
        ...
        if (rk_rawpkg == NULL) {
            printf("Failed to create key package for raw key\n");
            result = LD_ERR_KM_MALLOC; // 或其他适当的错误码
            break;
        }
        ...
        if (keypkg == NULL) {
            printf("Failed to create key package for storage\n");
            result = LD_ERR_KM_MALLOC; // 或其他适当的错误码
            break;
        }
...
derive_key(CCARD_HANDLE kdk_handle, enum KEY_TYPE key_type, uint32_t key_len, const char *owner1, const char *owner2,
    ...
    CCARD_HANDLE DeviceHandle, hSessionHandle;
    if (SDF_OpenDevice(&DeviceHandle) != SDR_OK) {
        printf("Error in derive: SDF_OpenDevice failed!\n");
        return NULL;
    }

    if (SDF_OpenSession(DeviceHandle, &hSessionHandle) != SDR_OK) {
        printf("Error in derive: SDF_OpenSession failed!\n");
        SDF_CloseDevice(DeviceHandle);
        return NULL;
    }
    ...
    if (!pkg) {
        printf("Error in derive: km_key_pkg_new failed!\n");
        return NULL;
    }
...
l_km_err km_derive_masterkey_asgs(uint8_t *db_name, uint8_t *table_name, CCARD_HANDLE handle_NH, uint32_t key_len,
                                  uint8_t *as_name, uint8_t *gs_name, uint8_t *rand, uint32_t rand_len,
                                  CCARD_HANDLE *handle_kasgs) {
    printf("deriving kasgs: gs_name: %s, as_name %s\n", gs_name, as_name); // amazing
...
km_derive_key(uint8_t *db_name, uint8_t *table_name, uint8_t *id, uint32_t key_len, uint8_t *gs_name, uint8_t *rand,
    ...
    // printf("dbname %s, tablename %s, id %s, asname: %s\n", db_name, table_name, id, qr_o->owner1);

    switch (query_keytype(db_name, table_name, id)) {
        case ROOT_KEY: {
            do {
                pkg = derive_key(handle, MASTER_KEY_AS_SGW, key_len, qr_o->owner1, qr_o->owner2, rand, rand_len, NULL);
                if (pkg == NULL) {
                ...
                uint16_t len_kasgs = 16;
                pkg_NH = derive_key(handle, NH_KEY, len_kasgs, qr_o->owner1, qr_o->owner2, rand, rand_len, &handle_NH);
                if (pkg_NH == NULL) {
                    printf("[**AS derive_key NH error**]\n");
                    break;
                }

                if (store_key(db_name, table_name, pkg_NH, &test_km_desc) != LD_KM_OK) {
                    break;
                }

                // printf("about to enter derive masterkey\n");
                // printf("dbname %s, tablename %s, id %s, asname: %s, gsname %s \n", db_name, table_name, id, owner1, gs_name);
                if (km_derive_masterkey_asgs(db_name, table_name, handle_NH, key_len, owner1, gs_name, rand, rand_len,
                                             NULL) != LD_KM_OK) {
 ...
 */
l_km_err
km_update_masterkey(uint8_t *dbname, uint8_t *tablename, uint8_t *sgw_name, uint8_t *gs_s_name, uint8_t *gs_t_name,
                    uint8_t *as_name, uint16_t len_nonce, uint8_t *nonce) {
    bool fail_tag = FALSE;
    QueryResult_for_update *qfu_masterkey = NULL;
    ...
    do {
        ...
        // 查询主密钥kas-gs id, only support active state to update
        ...
        // 查询待更新密钥kas-gs信息
        qfu_masterkey = query_for_update(dbname, tablename, qr_mk->ids[0]);
        ...
        if (qr_NH->count == 0) {
            printf("Query id_NH failed, null outcome.\n");
            fail_tag = TRUE;
            break;
        } else if (qr_NH->count > 1) // TODO: check ununique id
        {
            printf("Query id_NH failed, duplicate outcome.\n");
            fail_tag = TRUE;
            break;
        }
        // printf("Query id_NH OK,id:%s.\n", qr_NH->ids[0]);
        ...
        // printf("get kas-sgw handle_from_db OK\n");
        ...
        // 存储NH密钥值
        if (store_key(dbname, tablename, keypkg_NH, &test_km_desc) != LD_KM_OK) {
            printf("store_key failed.\n");
            fail_tag = TRUE;
            break;
        }
        ...
        keypkg_Kasgs = derive_key(handle_NH, MASTER_KEY_AS_GS, qfu_masterkey->key_len, as_name, gs_t_name, rand,
                                  rand_len,
                                  NULL);
        ...
        // 将原密钥的状态改为已经撤销
        if (km_revoke_key(dbname, tablename, qr_mk->ids[0]) != LD_KM_OK) {
            printf("km_revoke_key failed\n");
            fail_tag = TRUE;
            break;
        }
        ...
        if (qr_nmk->count == 0) {
            printf("Query new mkid failed, null kas-gs id.\n");
            fail_tag = TRUE;
            break;
        } else if (qr_nmk->count > 1) // TODO: check ununique id
        {
            printf("Query new mkid failed,kas-gs id is not unique.\n");
            fail_tag = TRUE;
            break;
        }
        // printf("Query mkid OK,id:%s.\n", qr_nmk->ids[0]);
        ...
        // 如果有子会话密钥
        qr_s = query_subkey(dbname, tablename, qr_mk->ids[0]);
        if (qr_s->count == 4) {
            // 会话密钥更新
    ...
    free_update_result(qfu_masterkey);
...
sgw_update_master_key(uint8_t *dbname, uint8_t *tablename, uint8_t *key_id, uint8_t *sgw_name, uint8_t *gs_t_name,
    ...
    do {
        ...
        if (qo->owner1 == NULL || qo->owner2 == NULL) {
            printf("query_owner failed\n");
            fail_tag = TRUE;
            break;
        }
        // printf("owner1: %s, owner2: %s\n", qo->owner1, qo->owner2);

        if (km_update_masterkey(dbname, tablename, sgw_name, qo->owner2, gs_t_name, qo->owner1, len_nonce, nonce) !=
            LD_KM_OK) {
            fail_tag = TRUE;
            break;
        }
...
l_km_err km_revoke_key(uint8_t *dbname, uint8_t *tablename, uint8_t *id) {
    ...
    do {
        ...
        // printf("dbname %s tablename %s key-id %s state: %s \n", dbname, tablename, id, kstate_str(state));
            ...
            if (km_generate_key_with_kek(kek_index, 16, &kek_handle, keypkg->kek_cipher, &keypkg->kek_cipher_len)) {
                ...
                if (km_generate_random(keypkg->iv, keypkg->iv_len)) {
                    ...
                    if (km_encrypt(kek_handle, ALGO_ENC_AND_DEC, iv_enc, key, meta->length, keypkg->key_cipher,
                        ...
                        // 校验算法
                        if (km_mac(kek_handle, ALGO_MAC, iv_mac, keypkg->key_cipher, cipher_len, keypkg->chck_value,
                                   &keypkg->chck_len)) {
