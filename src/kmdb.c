...
static int callback_for_query_keyvalue(void *data, int argc, char **argv, char **azColName) {
    ...
    // log_info("key cipher addr %p", key_cipher);
    // log_info("key  addr %p", key);
    ...
    // log_info("key  addr %p", key);
    // log_buf(LOG_WARN, "key", key, key_len);
    // log_info("key cipher addr %p", key_cipher);
    // log_info("key  addr %p", key);
...
QueryResult_for_keyvalue *query_keyvalue(uint8_t *db_name, uint8_t *table_name, uint8_t *id) {
    ...
    snprintf(sql, sizeof(sql),
             "SELECT kek_len, kek_cipher, key_len, key_cipher, iv_len, iv, chck_algo, check_len, chck_value  FROM %s WHERE id='%s';",
             table_name, id);
