...
static int queryid_callback(void *data, int argc, char **argv, char **azColName) {
    // printf("Callback called: argc = %d\n", argc);
    // printf("Callback called, argv[0]: %s\n", argv[0] ? argv[0] : "NULL");
...
static int callback_for_query_keyvalue(void *data, int argc, char **argv, char **azColName) {
    ...
    if (argc == 0) // 没有返回数据
    {
        printf("Error: No data returned from query\n");
    ...
    if (argc > 0 && argv[0] != NULL) {
        ...
        if (kek_len > 0) {
            ...
            if (kek == NULL) {
                printf("Error: Conversion of kek failed\n");
    ...
    if (argc > 3 && argv[3] != NULL) {
        if (key_len > 0) {
            ...
            if (key_cipher == NULL) {
                printf("Error: Conversion of key_cipher failed\n");
                free(kek);
                return LD_ERR_KM_CONVERT;
            }
        }
    ...
    if (argc > 5 && argv[5] != NULL) {
        if (iv_len > 0) {
            ...
            if (iv == NULL) {
                printf("Error: Conversion of iv failed\n");
                free(key_cipher);
                free(kek);
                return LD_ERR_KM_CONVERT;
            }
    ...
    if (result->key == NULL) {
        printf("Error: Memory allocation failed for result key\n");
        free(key);
        free(kek);
        free(key_cipher);
        free(iv);
...
QueryResult_for_keyvalue *query_keyvalue(uint8_t *db_name, uint8_t *table_name, uint8_t *id) {
    ...
    // 构建 SQL 查询语句
    char sql[512];
    snprintf(sql, sizeof(sql), "SELECT kek_len, kek_cipher, key_len, key_cipher, iv_len, iv FROM %s WHERE id='%s';",
             table_name, id);
